...
function WebChatbotListener() {
	...
	this.response = function(user, message) {
		this.pushToHistory(user, message);
		document.getElementById(this.prefix + 'response').innerHTML = SDK.linkURLs(message);
	...
	this.pushToHistory = function(userName, message){
		this.pushToDOM(userName, message);
		this.pushToDatabase(message);
	}

	this.pushToDOM = function(userName, message){
		var html;
		if(userName == "SerenityTn"){
			html = this.formatTextBot(message);
		}else{
			html = this.formatTextClient(userName, message);
		}
		$('#dialog-box').append(html);
		$('#dialog-box').scrollTop(1000000);
	}

	this.pushToDatabase = function(_message){
		$.post('dialog', {message: _message})
			.done(function(data) {
				console.log( "Data Loaded: " + data );
			});
	}

	this.formatTextBot = function(message){
		return html= "<div class='message message-bot'>"
							+"<img src='../imgs/olga.png'  width='50px' height='50px' alt='Olga' />"
							+"<b><u>Olga : </b></u>"+message+"<br/>";
	}

	this.formatTextClient = function(userName, message){
		return html= "<div class='message message-client'>"
							+"<img src='../imgs/user.png'  width='50px' height='50px' alt='client' />"
